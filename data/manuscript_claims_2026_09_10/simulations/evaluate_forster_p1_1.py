...
Output:
  simulations/forster_p1_1_robustness.json
"""
...
import optimize_bounded_minimax_forster_gate as minimax  # noqa: E402
...
OUT = ROOT / "simulations" / "forster_p1_1_robustness.json"
P0_RESULT = ROOT / "simulations" / "forster_p0_4_uncertainty_convergence.json"
...
def _build_mode(radius_um: float, direction_cosine: float):
    if radius_um == 0:
        ...
        model = minimax._build_geometry(
            radius_um, direction_cosine, p0.REFERENCE_BASIS
        ).model
...
def _thermal_summary(response) -> dict[str, object]:
    ...
    times_us = np.array(
        [0.0, target_start_us, target_midpoint_us, target_end_us, total_gate_us]
    )
    trajectory = ballistic_relative_trajectory(
        (0.0, 0.0, shaped.R0_UM), phase_space, times_us
    )
        ...
        trajectory_rows.append(
            {
                ...
                "axis_sigma_nm": (
                    1000 * np.std(offsets[:, index], axis=0, ddof=1)
                ).tolist(),
    ...
    return {
        ...
        "doppler_sigma_mhz": {
            "yb_302p043nm_along_weak_axis": shift_sigma(
                phase_space.yb.velocity_um_per_us, k_yb
            ),
...
def main() -> None:
    ...
    for radius_index, radius_um in enumerate(RADII_UM):
        if radius_um == 0:
            ...
            geometry_metadata.append(
                {"radius_um": 0.0, "direction_cosine": 0.0, **metadata}
            )
            continue
        for cosine_index, direction_cosine in enumerate(DIRECTION_COSINES):
            ...
            grid[radius_index, cosine_index] = _amplitude_grid(
                mode, pulse, lifetimes, correction
            )
    ...
    interpolated_values = np.asarray(
        response(_scenario_coordinates(scenarios)), dtype=float
    )
    ...
    for scenario_index in HOLDOUT_INDICES:
        ...
        interpolated = float(
            response(_scenario_coordinates([scenario])).item()
        )
        holdout_rows.append(
            {
    ...
    local_results = local_adversarial_search(
        ...
        worst_sample_seeds(
            scenarios, interpolated_values, LOCAL_SEED_COUNT
        ),
        DOMAIN,
    )
    ...
    training_vertex_minimum = float(
        np.min(grid[:, :, vertex_indices, :][:, :, :, vertex_indices])
    )
    ...
    result = {
        ...
        "response_grid": {
            ...
            "number_of_full_hamiltonian_geometries": len(geometry_metadata),
        ...
        "interpolation": {
            ...
            "role": (
                ...
                "candidates use the full P0-4 Hamiltonian"
            ),
            ...
            "rms_holdout_error": float(
                np.sqrt(
                    np.mean(
                        [row["interpolation_error"] ** 2 for row in holdout_rows]
                    )
                )
            ),
        },
        ...
        "local_adversarial_refinement": {
            ...
            "full_hamiltonian_rechecks": direct_candidates,
