...
Output:
  simulations/forster_p1_2_projection_audit.json
"""
...
OUT = ROOT / "simulations" / "forster_p1_2_projection_audit.json"
...
def _sparse_target_returns(
    ...
    *,
    ...
    mode_rates = (
        pp_weights * pp_rate
        + ss_weights * ss_rate
        + spectator_weights * max(pp_rate, ss_rate)
    )
    ...
    for segment in pulse:
        ...
        data = np.r_[
            ...
            -0.5j / (2 * np.pi * lifetimes.rb_56s_us),
            diagonal,
        ]
        ...
        spectator_population = max(
            0.0, pair_population - pp_population - ss_population
        )
...
def _gate_row(
    ...
    *,
    ...
    control_pi = shaped._control_pi(
        lifetimes.rb_56s_us, amplitude_scale=control_amplitude_scale
    )
    ...
    both = complex(
        (
            control_pi
            @ np.diag([target["unblocked_return"], target["blocked_return"]])
            @ control_pi
        )[0, 0]
    )
    kraus = np.array(
        [1.0, target["unblocked_return"], control_only, both], dtype=complex
    )
...
def _evaluate_point(
    ...
    sparse_kraus = np.array(
        [complex(*value) for value in projected["kraus_diagonal_re_im"]]
    )
    maximum_kraus_difference = float(
        np.max(np.abs(sparse_kraus - legacy_kraus))
    )
    ...
    if (
        maximum_kraus_difference > 1e-10
        or abs(fixed_fidelity_difference) > 1e-11
    ):
                ...
                np.max(
                    np.abs(
                        np.array(
                            [
                                complex(*value)
                                for value in all_modes["kraus_diagonal_re_im"]
                            ]
                        )
                        - sparse_kraus
                    )
...
def main() -> None:
    ...
    result = {
        ...
        "maximum_absolute_all_mode_projection_fidelity_difference": (
        ...
        "phase_and_frame_convention": {
            ...
            "rb_pulses": (
                "two positive 5 MHz pi pulses with the same optical phase phi=0"
            ),
            "aom": (
