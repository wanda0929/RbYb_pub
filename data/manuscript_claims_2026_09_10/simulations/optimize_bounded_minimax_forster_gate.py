...
Outputs:
  simulations/bounded_minimax_forster_gate_results.json
...
from style import DOUBLE_COL, use_paper_style  # noqa: E402

use_paper_style()


OUT = ROOT / "simulations" / "bounded_minimax_forster_gate_results.json"
...
def _historical_main(args) -> None:
    ...
    optimization: dict[str, object] = {
        ...
        "selected_pulse_provenance": {
            "script": "simulations/optimize_forster_p0_4_reference.py",
...
def _plot_result(result, figure_stem=FIGURE_STEM):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(DOUBLE_COL, 5.4), layout="constrained")
    ...
    axes[0, 1].remove()
    robustness_grid = robustness_spec.subgridspec(1, 3, wspace=0.12)
...
def _reference_main():
    ...
    paths = [ROOT / "simulations" / name for name in (
...
def main():
    ...
    if args.plot_only:
        ...
        _plot_result(result)
    elif args.optimize or args.historical_scan:
