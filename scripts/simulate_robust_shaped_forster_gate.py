...
the gate calculation but replaces the square Yb target pulse by a
...
basis. Loss uses non-Hermitian no-jump attenuation; state-resolved jump
branches and the destination of lost norm are not modeled.

Outputs:
  data/robust_shaped_forster_gate_results.json
...
OUT = ROOT / "data" / "robust_shaped_forster_gate_results.json"
...
def _square_pulse() -> tuple[Segment, ...]:
    return (Segment(SQUARE_OMEGA_MHZ, SQUARE_DETUNING_MHZ, SQUARE_DURATION_US),)
...
def _target_returns(
    ...
    for segment in pulse:
        ...
        unblocked_state = (
            expm(-2j * np.pi * h_unblocked * segment.duration_us * duration_scale) @ unblocked_state
        )
        ...
        if lifetimes is not None:
            ...
            spectator_weight = np.maximum(0.0, 1 - modes.pp_weights - modes.ss_weights)
            rates = (
        ...
        blocked_state = (
            expm(-2j * np.pi * h_blocked * segment.duration_us * duration_scale) @ blocked_state
        )
...
def _kraus_diagonal(
    ...
    control_idle = (
        1.0 if control_lifetime is None else np.exp(-target_duration / (2 * control_lifetime))
    )
    control_only = complex((control_pi @ np.diag([1.0, control_idle]) @ control_pi)[0, 0])
...
def _optimize_pulse(
    ...
    nominal_index = int(np.argmin([abs(mode.distance_um - R0_UM) for mode in modes]))
    evaluations = 0
...
def _metrics_for_pulse(
    ...
    return {
        ...
        "mean_computational_survival": float(metrics["mean_computational_survival"]),
        "conditional_phase_error_rad": float(metrics["conditional_phase_error_rad"]),
...
def main() -> None:
    ...
    if args.optimize:
        ...
        optimization_modes = _build_mode_grid(optimization_distances, SCAN_BASIS, threshold=1e-6)
    ...
    primary_modes = _build_mode_grid(ensemble_distances, PRIMARY_BASIS, threshold=1e-8)
    ...
    square_correction = _nominal_correction(primary_modes[nominal_index], square_pulse, lt0)
    ...
    for field in (B_GAUSS - 0.05, B_GAUSS + 0.05):
        ...
        field_fidelity = _evaluate_grid([field_mode], pulse, lt0, correction)[0]
    ...
    nominal_model = build_pair_model(B_GAUSS, PRIMARY_BASIS, distance_um=R0_UM, theta_deg=0.0)
    ...
    if args.verify_quadrature:
        ...
        modes_17 = _build_mode_grid(distances_17, SCAN_BASIS, threshold=1e-8)
        nominal_17 = int(np.argmin(abs(distances_17 - R0_UM)))
        correction_17 = _nominal_correction(modes_17[nominal_17], pulse, lt0)
        square_correction_17 = _nominal_correction(modes_17[nominal_17], square_pulse, lt0)
        robust_17 = _evaluate_grid(modes_17, pulse, lt0, correction_17)
        square_17 = _evaluate_grid(modes_17, square_pulse, lt0, square_correction_17)
    ...
    result = {
        ...
        "model": {
            ...
            "fidelity": (
                "loss-aware average CZ fidelity with one nominal local-Z correction held fixed"
            ),
            "decay": (
                "non-Hermitian no-jump attenuation; state-resolved jump branches "
                "and the destination of lost norm are not modeled"
            ),
