...
Outputs:
  data/channel_forster_gate_results.json
...
SIMULATION_DIR = ROOT / "scripts"
RESULT_PATH = ROOT / "data" / "channel_forster_gate_results.json"
...
PAIR_WINDOW_CHECK_BASIS = BasisConfig(delta_n=3, atomic_window_ghz=80.0, pair_window_ghz=10.0)
TARGET_TOTAL_M = 2.5
...
def _pair_state_indices(
    ...
    for ket in pair_basis.kets:
        ...
        rb_indices.append(rb_index_by_bare[rb_state.get_corresponding_ket_index()])
        yb_indices.append(yb_index_by_bare[yb_state.get_corresponding_ket_index()])
        m_value = float(rb_state.get_corresponding_ket().m + yb_state.get_corresponding_ket().m)
...
def _lift_rb_hyperfine_to_pair_basis(
    ...
    m_i_index = {round(float(value), 8): index for index, value in enumerate(RB87_M_I)}
    atomic_indices = np.asarray(
        [
            m_i_index[round(float(m_i), 8)] * rb_basis.number_of_states + rb_index
            for m_i, rb_index in zip(nuclear_m, rb_indices, strict=True)
        ],
        dtype=int,
    )
    rows = []
...
def build_pair_model(
    ...
    rb_system = pi.SystemAtom(rb_basis).set_magnetic_field([0, 0, field_gauss], unit="G")
    yb_system = pi.SystemAtom(yb_basis).set_magnetic_field([0, 0, field_gauss], unit="G")
    rb_system.diagonalize()
    yb_system.diagonalize()
    rb_ss_reference_mhz = float(rb_system.get_corresponding_energy(rb_ss, unit="MHz"))
    yb_ss_reference_mhz = float(yb_system.get_corresponding_energy(yb_ss, unit="MHz"))
    if electric_field_v_cm != 0.0:
        ...
        yb_system.diagonalize()
    rb_ss_carrier_shift_mhz = float(
        rb_system.get_corresponding_energy(rb_ss, unit="MHz") - rb_ss_reference_mhz
    )
    yb_ss_carrier_shift_mhz = float(
        yb_system.get_corresponding_energy(yb_ss, unit="MHz") - yb_ss_reference_mhz
    )
    ...
    pair_system.set_distance(distance_um, angle_degree=theta_deg, unit="micrometer")
    ...
    same_electronic_m = np.isclose(electronic_m[matrix.row], electronic_m[matrix.col], atol=1e-10)
    ...
    h_component -= defect_offset_mhz * np.outer(pp_component, pp_component.conj())
    ...
    static_transfer = {
        ...
        "spectator_population_at_maximum": float(
            max(
                0.0,
                1 - ss_population[maximum_index] - np.abs(pp_amplitude[maximum_index]) ** 2,
            )
        ),
    }

    pp_asymptote_mhz = (
        float(target_stretched_shift_mhz(56, 1, field_gauss) if include_rb_hyperfine else 0.0)
        - defect_offset_mhz
    )
    ss_asymptote_mhz = float(
        ...
        - energy_zero_mhz
        + (target_stretched_shift_mhz(56, 0, field_gauss) if include_rb_hyperfine else 0.0)
    )
...
def raman_propagator(lifetimes: Lifetimes | None) -> np.ndarray:
    ...
    h = np.array(
        [
            ...
            [
                PARAMS.omega_yb_optical_mhz / 2,
                PARAMS.raman_delta_mhz,
                PARAMS.omega_yb_microwave_mhz / 2,
            ],
...
def blockaded_return(
    ...
    if lifetimes is not None:
        ...
        mode_rates = (
            pp_weight * pp_rate + ss_weight * ss_rate + spectator_weight * max(pp_rate, ss_rate)
        )
...
def _local_z_metrics(kraus_diagonal: np.ndarray) -> dict[str, float | list[float]]:
    ...
    average_fidelity = float((4 * survival + abs(overlap) ** 2) / (4 * (4 + 1)))
    conditional_phase = float(
        np.angle(kraus_diagonal[3] * kraus_diagonal[0] / (kraus_diagonal[2] * kraus_diagonal[1]))
    )
...
def simulate_gate(
    ...
    blocked = blockaded_return(model, omega_rb_mhz, lifetimes, mode_threshold=mode_threshold)
    ...
    middle_11 = np.diag(
        [
            unblocked,
            unblocked * np.exp(0 if coherent else -0.5 / yb_ss_lifetime / omega_rb_mhz),
            blocked,
        ]
    )
    kraus = np.array(
    ...
    metrics.update(
        {
            ...
            "total_sequence_time_us": float(2 * PARAMS.raman_pi_time_us + 1 / omega_rb_mhz),
            ...
            "kraus_diagonal_re_im": [[float(value.real), float(value.imag)] for value in kraus],
...
def optimize_omega(
    ...
    fidelities = np.array(
        [
            simulate_gate(model, omega, lifetimes, mode_threshold)["average_gate_fidelity"]
    ...
    candidates.extend(
        ...
        if fidelities[index] >= fidelities[index - 1] and fidelities[index] >= fidelities[index + 1]
    )
...
def _verify_average_fidelity_formula(metrics: dict[str, object]) -> float:
    kraus = np.array([complex(real, imag) for real, imag in metrics["kraus_diagonal_re_im"]])
...
def _target_trace(
    ...
    initial_coefficients = np.linalg.solve(eigenvectors, np.r_[1, np.zeros(generator.shape[0] - 1)])
    states = (
        eigenvectors @ (initial_coefficients[:, None] * np.exp(np.outer(eigenvalues, times)))
    ).T
...
def _plot_figure(
    ...
    curve_coherent = [
        simulate_gate(primary, value, None, 1e-6)["average_gate_fidelity"] for value in curve_omega
    ]
    ...
    ax = axes[0, 1]
    ax.plot(
        trace["time_us"], trace["ground"], label=r"$|1_{\rm Rb},r_{\rm Yb}\rangle$", color=COLORS[0]
    )
...
def run_simulation() -> dict[str, object]:
        ...
        field_scan.append(
            {
                ...
                "static_maximum_ss_population": model.static_transfer["maximum_ss_population"],
            }
        )
        print(
    ...
    final_optimization = minimize_scalar(
        lambda omega: -simulate_gate(primary, omega, lifetimes_300, 1e-8)["average_gate_fidelity"],
    ...
    full_mode_blocked = full_mode_blockaded_return_sparse(primary, optimized_omega, lifetimes_300)
    ...
    mode_difference = abs(optimized["average_gate_fidelity"] - mode_check["average_gate_fidelity"])
    ...
    for fraction in [-0.02, -0.01, 0.0, 0.01, 0.02]:
        metrics = simulate_gate(primary, optimized_omega * (1 + fraction), lifetimes_300, 1e-8)
        sensitivity.append(
            {
    ...
    result: dict[str, object] = {
        ...
        "model_scope": {
            ...
            "decay": (
                "non-Hermitian no-jump attenuation at the stated temperature; "
                "state-resolved jump branches and the destination of lost norm are not modeled"
            ),
            "not_included": [
        ...
        "convergence": {
            ...
            "bright_mode_threshold_1e-6_fidelity": mode_check["average_gate_fidelity"],
            ...
            "full_mode_minus_projected_amplitude": float(full_mode_amplitude_difference),
        },
        "validation": {
            ...
            "analytic_minus_monte_carlo": float(optimized["average_gate_fidelity"] - monte_carlo),
        },
    }
