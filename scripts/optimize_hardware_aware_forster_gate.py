...
Outputs:
  data/hardware_aware_forster_gate_results.json
...
OUT = ROOT / "data" / "hardware_aware_forster_gate_results.json"
...
def _geometry_grid(config) -> list[Geometry]:
    ...
    geometries = []
    for delta_z, axial_weight in zip(axial_values, axial_weights, strict=True):
        for transverse, transverse_weight in zip(
            transverse_values, transverse_weights, strict=True
        ):
...
def _fidelity(modes, pulse, lifetimes, correction, **errors) -> float:
    return shaped._fixed_correction_fidelity(_kraus(modes, pulse, lifetimes, **errors), correction)
...
def _evaluate_ensemble(
    ...
    **errors,
    ...
    fidelities = np.array(
        [_fidelity(mode, pulse, lifetimes, correction, **errors) for mode in modes]
    )
...
def _optimize(
    ...
    def objective(parameters: np.ndarray) -> float:
        ...
        evaluations += 1
        pulse = _filtered_pulse(parameters, AOM_RISE_TIME_NS, OPTIMIZATION_STEP_NS)
        ...
        nominal = _fidelity(modes[nominal_index], pulse, lifetimes, correction)
...
def main() -> None:
    ...
    optimization_modes = [shaped._prepare_modes(geometry.model, 1e-3) for geometry in geometries]
    ...
    final_modes = [shaped._prepare_modes(geometry.model, 1e-6) for geometry in geometries]
    ...
    nominal_0k = _fidelity(final_modes[nominal_index], pulse, lt0, correction)

    original_pulse = _filtered_pulse(shaped.SELECTED_PARAMETERS, AOM_RISE_TIME_NS, FINAL_STEP_NS)
    original_correction = _correction(final_modes[nominal_index], original_pulse, lt0)
    ...
    for label, errors in {
        ...
        ensemble, curve = _evaluate_ensemble(final_modes, weights, pulse, lt0, correction, **errors)
        robustness[label] = {
    ...
    for rise_time in (2.0, 5.0, 10.0):
        ...
        filtered_correction = _correction(final_modes[nominal_index], filtered, lt0)
        ensemble, curve = _evaluate_ensemble(
            final_modes, weights, filtered, lt0, filtered_correction
        )
        rise_time_scan[str(rise_time)] = {
            "target_duration_us": float(sum(segment.duration_us for segment in filtered)),
            "nominal_fidelity": float(curve[nominal_index]),
            "ensemble_fidelity": ensemble,
            "worst_quadrature_fidelity": float(np.min(curve)),
        }
    ...
    for label, lifetimes in (("300k", lt300), ("coherent", None)):
        ensemble, curve = _evaluate_ensemble(final_modes, weights, pulse, lifetimes, correction)
        temperature[label] = {
            "nominal_fidelity": float(curve[nominal_index]),
            "ensemble_fidelity": ensemble,
            "worst_quadrature_fidelity": float(np.min(curve)),
    ...
    result = {
        ...
        "assumptions": {
            ...
            "geometry_quadrature": (
                "three-point Gauss-Hermite per Cartesian axis; nine unique cylindrical points"
            ),
            "rb_excitation": (
                "effective two-level 5 MHz pi pulse; intermediate-state scattering omitted"
            ),
            "decay": (
                "PairInteraction lifetime-weighted no-jump attenuation; state-resolved "
                "jump branches and the destination of lost norm are not modeled"
            ),
        ...
        "selected_10ns": {
            ...
            "worst_quadrature_fidelity_0k": float(np.min(geometry_fidelities)),
        ...
        "geometry": [
            ...
            for geometry, fidelity in zip(geometries, geometry_fidelities, strict=True)
        ],
    }
    ...
    ax = axes[0]
    command_edges = np.r_[0, np.cumsum([segment.duration_us for segment in command])]
    ax.stairs(
    ...
    ax = axes[1]
    labels = [
        f"{geometry.delta_z_um * 1000:+.0f}\n{geometry.theta_deg:.1f}°" for geometry in geometries
    ]
    ax.plot(
    ...
    ax.set_xticks(np.arange(len(bar_labels)), bar_labels, rotation=48, ha="right")
    ax.set_ylabel(r"$10^4(1-F_{\rm avg})$")
