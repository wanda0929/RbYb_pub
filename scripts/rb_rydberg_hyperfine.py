...
def hyperfine_constants_mhz(
    ...
    prefactors = (
        f_state_prefactors_mhz if int(l) == 3 else _HFS_PREFACTORS_MHZ.get((int(l), float(j)))
    )
...
def _angular_momentum_matrices(
    ...
    for column, m in enumerate(magnetic_numbers):
        ...
        if row is not None:
            plus[row, column] = np.sqrt(angular_momentum * (angular_momentum + 1) - m * (m + 1))
...
def isolated_hyperfine_hamiltonian_mhz(
    ...
    x = np.kron(i_z, j_z) + 0.5 * np.kron(i_plus, j_minus) + 0.5 * np.kron(i_minus, j_plus)
    a_mhz, b_mhz = hyperfine_constants_mhz(n, l, j, f_state_prefactors_mhz=f_state_prefactors_mhz)
    hamiltonian = a_mhz * x
    if b_mhz and j >= 1:
        ...
        numerator = (
            3 * (x @ x)
            + 1.5 * x
            - RB87_NUCLEAR_SPIN * (RB87_NUCLEAR_SPIN + 1) * j * (j + 1) * identity
        )
        denominator = 2 * RB87_NUCLEAR_SPIN * (2 * RB87_NUCLEAR_SPIN - 1) * j * (2 * j - 1)
        hamiltonian = hamiltonian + b_mhz * numerator / denominator
    nuclear_zeeman = (
        RB87_G_I_BOHR * MU_B_OVER_H_MHZ_PER_G * field_gauss * np.kron(i_z, np.eye(len(m_j)))
    )
...
def target_stretched_shift_mhz(n: int, l: int, field_gauss: float) -> float:
    ...
    return float(
        ...
        + RB87_G_I_BOHR * MU_B_OVER_H_MHZ_PER_G * field_gauss * RB87_STRETCHED_M_I
    )
...
def dressed_hyperfine_hamiltonian_mhz(
    ...
    for (n, l, twice_j), electronic_indices in blocks.items():
        ...
        for local_row, local_column in zip(local_rows, local_columns, strict=True):
            nuclear_row, electronic_row = divmod(local_row, local_electronic_size)
            nuclear_column, electronic_column = divmod(local_column, local_electronic_size)
            rows.append(nuclear_row * number_electronic + electronic_indices[electronic_row])
            columns.append(
                nuclear_column * number_electronic + electronic_indices[electronic_column]
            )
            values.append(local[local_row, local_column])

    nuclear_diagonal = np.repeat(
        RB87_G_I_BOHR * MU_B_OVER_H_MHZ_PER_G * field_gauss * RB87_M_I,
        number_electronic,
    )
